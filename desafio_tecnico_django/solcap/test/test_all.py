...
class AllTestCase(APITestCase):
	...
	def setUp(self):
		...
		self.list_url = 'http://localhost:8000/all/'
	...
	def	test_get_all(self):
		...
		response = self.client.get('http://localhost:8000/all/')
	...
	def	test_del_all(self):
		...
		response = self.client.delete('http://localhost:8000/all/1/')
		self.assertEquals(response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)
	
	def test_put_all(self):
		"""Teste para verificar a requisição PUT quando uma pessoa é atualizada"""
		data = {
			'nome':'Cezar Augusto',
			'cpf':'111.222.333-44',
			'rg':'11.222.333-4',
		...
		response = self.client.put('http://localhost:8000/all/2/', data=data)
		self.assertEquals(response.status_code, status.HTTP_200_OK)