...
class AllTestCase(APITestCase):
	...
	def setUp(self):
		...
		self.list_url = 'http://localhost:8000/olders/'
	...
	def	test_get_olders(self):
		"""Teste GET para verificar a requisição GET para listar as pessoas"""
		response = self.client.get('http://localhost:8000/olders/')
		self.assertEquals(response.status_code, status.HTTP_200_OK)
	
	def	test_post_olders(self):
		...
		response = self.client.post('http://localhost:8000/olders/', data=data)
		self.assertEquals(response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)

	def	test_del_olders(self):
		"""Teste para verificar a requisição DELETE não permitida para deletar uma pessoa"""
		response = self.client.delete('http://localhost:8000/olders/1/')
		self.assertEquals(response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)
	
	def test_put_olders(self):
		...
		response = self.client.put('http://localhost:8000/olders/1/', data=data)
		self.assertEquals(response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)